...
$(document).on('input','#conge_date_demande',function(){
     ...
     var getUrl = Routing.generate("workers_change");

     $.post(getUrl,data).then(function(data){

        //  let $input = $(data).find('#personnel_categorie')
         $('#conge_motif').replaceWith(data)
         
     })
});

$(document).on('change','#conge_motif',function(){
    let $field = $(this)
    let $form = $field.closest('form')
    let data = {}
    data[$field.attr('name')] = $field.val()

    var getUrl = Routing.generate("motif_change");

    $.post(getUrl,data).then(function(data){

       //  let $input = $(data).find('#personnel_categorie')
        $('#conge_qte_dispo').replaceWith(data)
